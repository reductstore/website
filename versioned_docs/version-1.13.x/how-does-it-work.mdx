...
- The **[HTTP API](./http-api)** provides a simple and portable way to communicate with the storage engine.
...
- **Data Querying.** You can filter records by labels using the [**Conditional Query**](/docs/conditional-query/index.mdx) language.
...
ReductStore provides an [**HTTP API**](/docs/http-api/index.mdx) for interacting with the storage engine. As mentioned above, the ReductStore HTTP API has the following features
...
- **Filtering Records by Labels.** A client can filter records by labels using the [**Conditional Query**](/docs/conditional-query/index.mdx) language.
...
- **Filtering.** The storage engine supports filtering records using the [**Conditional Query**](/docs/conditional-query/index.mdx) language, so that you can replicate only records that match the query.
