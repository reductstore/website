...
MCAP is a container format for recorded robotics data. It can store ROS1, ROS2, and Protobuf messages across many topics together with timestamps, schemas, and channel metadata. ReductROS can extract and transform ROS1, ROS2, and Protobuf messages from MCAP files stored in ReductStore (content type `application/mcap`).
...
:::info
MCAP extraction supports ROS1 (`ros1msg`), ROS2 (`cdr`), and Protobuf (`protobuf`) schema encodings. Mixed MCAP files are supported, so topics using different encodings can coexist in the same input file and still be extracted correctly topic by topic. For Protobuf channels, the schema must be a `FileDescriptorSet` and the `schema` label in the output is the fully qualified message type, for example `factory.EnvironmentReading`.
:::
