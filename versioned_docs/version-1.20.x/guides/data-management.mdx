...
<Tabs>
  ...
  <TabItem value="C++">
    ...
    10. Click on the **+** icon in the record row to open the edit panel:<br /><img
        src={
            require("./img/data_management_update_label.png").default
        }
        alt="Edit record with Web Console"
        width="800"
    />
    11. In the edit panel you can add, remove or update labels for the record.
    12. Click on the **Save** icon to save the changes.
...
**[Attachments](../glossary#attachment)** let you store shared JSON metadata for an **[entry](../glossary#entry)** without duplicating it in each record label.
They are stored in the hidden `<entry_name>/$meta` system entry.

This is useful when metadata changes less frequently than record data. For example, you can keep one schema, camera calibration profile, or model configuration per entry and reuse it for all records in that entry.
Because attachments are regular records in a system entry, they are replicated together with the parent entry data in replication workflows.

You can manage attachments using the ReductStore SDKs, the Reduct **[CLI](../glossary#cli)**, the **[Web Console](../glossary#web-console)**, or the **[HTTP API](../glossary#http-api)**.

import ManageAttachmentsPy from "!!raw-loader!../examples/py/src/data_management_attachments.py";
import ManageAttachmentsJs from "!!raw-loader!../examples/js/src/data_management_attachments.mjs";
import ManageAttachmentsGo from "!!raw-loader!../examples/go/src/data_management_attachments.go";
import ManageAttachmentsRs from "!!raw-loader!../examples/rs/examples/data_management_attachments.rs";
import ManageAttachmentsCpp from "!!raw-loader!../examples/cpp/src/data_management_attachments.cc";
import ManageAttachmentsCli from "!!raw-loader!../examples/cli/data_management_attachments.sh";

<Tabs>
  <TabItem value="CLI">
    <CodeBlock language="bash">{ManageAttachmentsCli}</CodeBlock>
  </TabItem>
  <TabItem value="Python">
    <CodeBlock language="python">{ManageAttachmentsPy}</CodeBlock>
  </TabItem>
  <TabItem value="JavaScript">
    <CodeBlock language="javascript">{ManageAttachmentsJs}</CodeBlock>
  </TabItem>
  <TabItem value="Go">
    <CodeBlock language="go">{ManageAttachmentsGo}</CodeBlock>
  </TabItem>
  <TabItem value="Rust">
    <CodeBlock language="rust">{ManageAttachmentsRs}</CodeBlock>
  </TabItem>
  <TabItem value="C++">
    <CodeBlock language="cpp">{ManageAttachmentsCpp}</CodeBlock>
  </TabItem>
</Tabs>

For low-level requests and `$meta` rules, see the **[Attachments HTTP API](../http-api/entry-api/attachments.mdx)**.

:::warning
Attachment keys starting with `$` are reserved for official ReductStore plugins and tools. Use keys without a leading `$` for user-defined attachments.
:::

## Deleting Data

The ReductStore API allows you to delete specific records from an entry, an entire entry, or an entire **[bucket](../glossary#bucket)**.
You can do this using the Client SDKs, the Reduct CLI, or the HTTP API.

### Automatic Deletion With Lifecycle Policies

For automatic cleanup, use lifecycle policies. A delete lifecycle policy periodically
removes records older than `older_than` from a bucket. You can limit cleanup to selected
entries with `entries` and to matching records with a `when` condition. Use `dry_run`
mode to preview how many records match before enabling deletion.

For a full explanation and provisioning examples, see the
**[Lifecycle Policies](lifecycle-policies.mdx)** guide.
...
<Tabs>
  ...
  <TabItem value="Web Console">
    ...
    ![Browse buckets with Web Console](./img/entry_rename_select_bucket.png)
    4. You will see a list of all the entries in the bucket
    ...
    ![Remove entry with Web Console](./img/entry_remove.png)
...
:::info
Pay attention that removing a bucket doesn't remove it immediately.
The bucket is marked as **DELETING**, and the cleanup continues in the background.
During this time, you cannot access the bucket or its entries.
Once the deletion is complete, the bucket and all its data are permanently removed from the store.
:::
...
<Tabs>
  <TabItem value="CLI">
    ...
    7. Enter the time range in the date picker or manually in the input fields
    8. Click the **Fetch Records** button to execute the query
    9. Click on the **Delete** icon next to the record you want to delete:
    ![Delete record with Web Console](./img/data_management_delete_record.png)
...
<Tabs>
  ...
  <TabItem value="Web Console">
    ...
    ![Edit entry with Web Console](./img/entry_rename_edit.png)
    6. Rename the entry and click on the **Save** button:
    ![Save entry name with Web Console](./img/entry_rename_save.png)
