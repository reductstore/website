...
Starting with version 1.18, you can query records across multiple entries by providing the `entries` parameter with a list of entry names or patterns.
Exact entry names and flat trailing-prefix patterns such as `cam-*` keep their existing behavior.
For path-like names, `*` matches one segment, `**` matches recursively across segments, and patterns prefixed with `!` exclude entries after includes are resolved.
For example, `["/a/**/b", "!/a/private/**", "cam-*"]` queries entries such as `/a/x/b` and `cam-1`, but excludes `/a/private/x/b`.
:::
