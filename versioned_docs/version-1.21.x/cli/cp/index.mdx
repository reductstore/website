...
- `-b, --start <start>`: Start timestamp (inclusive) as Unix microseconds, RFC3339, local date/datetime, or relative time (e.g. now - 1h). If not specified, the export will start from the first record in each entry.
...
- `-e, --stop <stop>`: Stop timestamp (inclusive) as Unix microseconds, RFC3339, local date/datetime, or relative time (e.g. now - 1h). Defaults to now.
...
Options:
  -b, --start <start>                Start timestamp (inclusive) as Unix microseconds, RFC3339, local date/datetime, or relative time (e.g. now - 1h).
