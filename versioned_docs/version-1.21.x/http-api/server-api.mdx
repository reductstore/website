...
- Version 1.18: bucket responses include the `status` field (`READY` / `DELETING`).
    ...
    "buckets": [
      {
        ...
        "oldest_record": "integer", // unix timestamp of oldest record in microseconds
        "latest_record": "integer",  // unix timestamp of latest record in microseconds
        "status": Union["READY", "DELETING"] // resource status
      }
    ]
}`}
            </code>
...
If the lock file mechanism is enabled (for example, `RS_INSTANCE_ROLE=PRIMARY` or `RS_INSTANCE_ROLE=SECONDARY`), the storage engine uses a lock file to prevent multiple instances from accessing the same data directory simultaneously.
...
<SwaggerComponent
  method="get"
  path="/api/v1/ready"
  summary="Check if the storage engine is active"
  responses={[
    {
      status: "200",
      message: "OK",
