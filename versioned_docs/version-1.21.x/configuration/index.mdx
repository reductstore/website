...
| `RS_PUBLIC_URL`        | http(s)://[RS_HOST]:[RS_PORT][RS_API_BASE_PATH] | Public URL of the instance. Used for **[query links](/docs/glossary#query-link)**.                                                                      |
| `RS_PRIMARY_URL`       |                                                 | Public URL of the primary ReductStore instance used when forwarding system information (for example, audit events).                                     |
| `RS_SECONDARY_URL`     |                                                 | Optional fallback URL for the primary instance.                                                                                                         |
| `RS_API_TOKEN`         |                                                 | Enables authentication and provisions the `init-token` with this value. The token cannot be rotated or removed through the API.                         |
| `RS_INIT_API_TOKEN`    |                                                 | Enables authentication and initializes a manageable `init-token` with this value. The token can later be rotated or removed through the API.            |
| `RS_DISABLE_AUTH`      | false                                           | Set to `true` to explicitly run without authentication.                                                                                                 |
...
| `RS_LICENSE`           |                                                 | Inline license document content. Required for commercial usage unless `RS_LICENSE_PATH` is set. See **[Pricing](/pricing)** for more details.           |
| `RS_LICENSE_PATH`      |                                                 | Path to the license file. Required for commercial usage unless `RS_LICENSE` is set. See **[Pricing](/pricing)** for more details.                       |
| `RS_LICENSE_KEY`       |                                                 | License key from a ReductPro subscription. The instance validates it with the ReductStore license server. See **[Pricing](/pricing)** for more details. |
...
| `RS_INSTANCE_NAME`     |                                                 | Optional instance name used in audit record labels (`instance`). If unset, ReductStore uses OS hostname (or `unknown` if unavailable).                  |
...
Commercial deployments need a valid license. You can provide it in one of three ways:

- Set `RS_LICENSE_KEY` to the license key from your ReductPro subscription. The instance validates the key with the ReductStore license server.
...
{/* TODO: confirm with Alexey whether RS_LICENSE_KEY may be combined with RS_LICENSE or RS_LICENSE_PATH, and which source takes precedence. */}
