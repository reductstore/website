...
description: Using S3-compatible and Azure Blob storage as backends for ReductStore.
---
...
<head>
  <link
    rel="canonical"
    href="https://www.reduct.store/docs/integrations/cloud-storage"
  />
</head>

This tutorial walks through using a cloud object storage backend for ReductStore in three deployment patterns:
...
## Shared cloud-storage configuration (S3 examples)
...
RS_REMOTE_SESSION_TOKEN=<your-session-token> # Optional, for temporary credentials
RS_REMOTE_CACHE_PATH=/var/reduct/cache
RS_REMOTE_CACHE_SIZE=10GB          # Adjust based on your workload
RS_REMOTE_SYNC_INTERVAL=60        # Adjust based on ingestion rate
```

If you run on AWS and want to use the default credential provider chain, omit `RS_REMOTE_ACCESS_KEY` and `RS_REMOTE_SECRET_KEY` entirely. If you provide one of them, you must provide the other one too. Use `RS_REMOTE_SESSION_TOKEN` only with temporary credentials.

For commercial deployments, provide your enterprise license either as a mounted file with `RS_LICENSE_PATH` or as inline secret content with `RS_LICENSE`. Do not set both variables at the same time.

## Azure differences

To use Azure Blob Storage with the same topologies, keep the same role/topology variables and switch only the remote backend settings:

- Set `RS_REMOTE_BACKEND_TYPE=azure`
- `RS_REMOTE_BUCKET` is the Azure **container** name
- Authentication:
  - Account key mode: `RS_REMOTE_ACCESS_KEY=<storage-account-name>` and `RS_REMOTE_SECRET_KEY=<storage-account-key>`
  - SAS mode: `RS_REMOTE_ACCESS_KEY=<storage-account-name>` and `RS_REMOTE_SAS_TOKEN=<sas-token>`
- `RS_REMOTE_ENDPOINT` is optional (required for custom endpoints such as Azurite)
- `RS_REMOTE_REGION` and `RS_REMOTE_SESSION_TOKEN` are S3-specific and not used for Azure

## Standalone on cloud storage

Standalone mode is the simplest way to combine ReductStore with cloud storage. You run a single instance that writes data to S3 and keeps a local cache for fast access to recent records. There is no lock file or coordination with other nodes, so you only manage one container and one set of credentials. If you are not sure which pattern to pick, start here.
...
## Active-passive on cloud storage
...
## Read-only replica on cloud storage
...
services:
  reduct-replica:
    ...
    environment:
      ...
      RS_ENGINE_REPLICA_UPDATE_INTERVAL: 30 # Refresh indexes and changes from S3 every 30 seconds
      RS_LICENSE_PATH: /var/reduct/license/license.key
    volumes:
