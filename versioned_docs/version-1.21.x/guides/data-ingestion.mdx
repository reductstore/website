...
:::info
For small records, ReductStore provides a batch ingestion mode. In this mode, the server receives multiple records in a single request to reduce HTTP overhead. Use the [**I/O API**](../http-api/io-api#batch-protocol-v2) when a batch must write records to multiple entries.
:::
...
- ReductStore doesn't have any limitations on the size of a record. However, record metadata is sent in HTTP headers, and HTTP/1.1 clients, proxies, or servers can reject requests when headers become too large. For batched I/O requests, ReductStore limits header metadata with the [**I/O settings**](../configuration/settings#io-settings) `RS_IO_BATCH_MAX_METADATA_SIZE` and `RS_IO_BATCH_MAX_RECORDS`.
...
<Tabs>
    ...
    <TabItem value="Web Console">
        ...
        ![Browse a bucket with Web Console](./img/buckets_upload_file_icon.png)
        7. In the upload file modal, select the entry name or create a new one and select the file to upload:<br /><img
        src={
            require("./img/buckets_upload_file.png").default
        }
...
For smaller records, it is recommended to use batch ingestion mode. This mode allows a client application to send a batch of records across multiple entries in a single request, reducing the HTTP protocol overhead.

The following snippets write records for `sensor-1` and `camera-1` in one batch request.
...
:::caution HTTP/1.1 header limits
Batch metadata is encoded in HTTP headers. For HTTP/1.1 clients, keep the number of records and metadata size within `RS_IO_BATCH_MAX_RECORDS` and `RS_IO_BATCH_MAX_METADATA_SIZE`. If a batch has many records, labels, or long entry names, reduce the batch size or adjust these [**I/O settings**](../configuration/settings#io-settings).
:::
