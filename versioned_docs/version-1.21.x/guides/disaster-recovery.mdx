...
- The corrupted block is removed from the in-memory index and will be ignored on subsequent requests until restart.
  On restart, the corrupted block is removed only if integrity checks are enabled; otherwise, it must be removed manually or the issue will repeat.
...
Due to architectural constraints of the ReductStore storage engine, **file backups can only be created or restored when the active instance is stopped**.
...
2. Start a new ReductStore instance with the copied data:
   ```bash
   RS_DATA_PATH=/path/to/backup RS_API_TOKEN=my-token reductstore
   ```
3. Check the logs for any startup errors.
4. Verify data accessibility, for example:
   ```bash
   curl --header "Authorization: Bearer my-token" http://127.0.0.1:8383/api/v1/list
   ```
...
This model requires failover logic that can switch traffic in your load balancer or reverse proxy, including solutions that support backup upstreams (for example, NGINX).
...
For an end-to-end active-passive setup on cloud storage, see **[Cloud Storage Integration](../integrations/cloud-storage#active-passive-on-cloud-storage)**.
...
![Read-only Replicas](./img/disaster_recovery_read_replicas.png)
...
- `RS_ENGINE_REPLICA_UPDATE_INTERVAL`: How often replicas refresh bucket metadata and indexes from the backend.
- `RS_ENGINE_COMPACTION_INTERVAL`: How often the writer compacts WALs into blocks and syncs them to the backend (in addition to syncing when a block is finished).
...
For a full walkthrough using cloud storage, see **[Cloud Storage Integration](../integrations/cloud-storage#read-only-replica-on-cloud-storage)**.
