...
description: How to set up local storage on mobile robots with automatic cloud replication tasks using ReductStore.
authors: anthony
date: 2025-11-17
...
Mobile robots produce a **lot** of data (camera images, IMU readings, logs, etc). Storing this data reliably on each robot and syncing it to the cloud can be hard. **ReductStore** makes this easier: it's a lightweight, time-indexed object store built for robotics and industrial IoT. It stores binary blobs (images, logs, CSV sensor data, MCAP, JSON) with timestamps and labels so you can quickly find and query them later.
