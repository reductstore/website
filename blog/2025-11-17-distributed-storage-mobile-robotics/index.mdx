...
- It sends them to the cloud in batches.
...
This can be configured per robot using environment variables (provisioning), with the web console or via the CLI (as shown below).
...
This starts a ReductStore server that writes to S3 automatically. There are many more configuration options available in the **[configuration documentation](https://www.reduct.store/docs/configuration)**.
...
That starts a ReductStore server on port `8383` by default. Then you can use the CLI to set up replication task to the cloud:
...
reduct-cli replica create local robot1-to-cloud \
    robot1-data https://<CLOUD_API_TOKEN>@<cloud-address>/robot1-data
```

This creates a replication task called `robot1-to-cloud` that copies all data from the robot's local `robot1-data` bucket to the cloud instance.

## Storing Sensor Data

There are many ways to store data. When it comes to high-frequency sensor data like IMU readings, a common approach is to store them in 1-second files. Images can be stored as binary blobs (e.g., JPEG or PNG files). Here is an example of storing IMU data as CSV files and images as binary blobs using the Python SDK (this stores 10,000 IMU samples and one camera image for a given timestamp as an example):
...
async def main():
    async with Client("http://localhost:8383", api_token="<ROBOT_API_TOKEN>") as client:
        bucket = await client.get_bucket("robot1-data", create=True)
...
This returns only the rows where `linear_acceleration_x > 10`, along with the timestamp.
...
- **Robots can store data on disk** and operate offline without network connectivity
- **Automatic replication when connected** to avoid manual uploads and simplify data management
- **Selective replication** lets you control what data is sent to the cloud (reduction strategy) to save bandwidth and storage
