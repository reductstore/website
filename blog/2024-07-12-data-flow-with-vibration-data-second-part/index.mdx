...
Replication tasks can be configured in multiple ways, such as:
- Replicating all data from a source bucket to a destination bucket (e.g., `sensor_data` to `backup_data`)
- Replicating only specific entries, e.g., `sensor_readings`
- Filtering data based on labels, e.g., replicating only data with `high` RMS values

Replication task can be set using client libraries, HTTP API, provisioning, or the ReductStore web console.
In this example, we'll use the web console to set up replication but you can also look at the **[Replication Guide](/docs/next/guides/data-replication)** for more details.
...
3. Enter a name for your replication task and add the following details:
    ...
    - Optionally, set up a filter on the entries to replicate (e.g., `sensor_readings`). When not set, all entries will be replicated.
    ...
    - Then, set up filters based on labels (e.g., to replicate only data with "high" RMS).
        <img 
...
With this new replication task, all new data from the `sensor_data` bucket with the label "high" for peak-to-peak values will be replicated to the `high_peak_to_peak` bucket.
...
5. Finally, we explored how to set up replication using the ReductStore web console so that we could copy high peak-to-peak metrics into a separate bucket.
...
Feel free to join us on [**Discord**](https://discord.com/invite/8wPtPGJYsn) for a quick chat, or start a discussion on [**GitHub**](https://github.com/reductstore/reductstore/discussions).