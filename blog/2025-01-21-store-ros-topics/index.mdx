...
- **Offline analysis**: There is no need to connect to the live robot in order to analyze the recorded data.
  **Drawbacks**:
- **Limited scalability**: Managing and storing large Rosbag files can become a bit difficult as the amount of data grows.
...
Another option is to store compressed Rosbag files in a _time-series object storage solution_ like [**ReductStore**](/). Time-series storage works really well for ROS data because it sorts and retrieves information by time, which fits naturally with how robots process and use data. This approach also lets you add helpful labels to the files, like timestamps, sensor types, or recording locations, making it easier to manage and access your data.

**Advantages**:

- **Advanced filtering**: Labels make it easier to search and filter large datasets based on specific criteria.
- **Scalable**: ReductStore can easily handle large amounts of data, making it suitable for long-term storage of robotic data.
  **Drawbacks**:
...
- **Conditional filtering and replication**: With the ability to filter records based on metadata, you can quickly find what you need. You can also have granular replication by replicating only the important topics to the cloud.
  **Drawbacks**:
...
When choosing how to store ROS topics, consider data size, query needs, and scalability.

- **Data Size and Scalability**: For large datasets like LiDAR or images, ReductStore's time-series storage is ideal. Simpler solutions like Rosbag are enough for smaller data but may become difficult to manage as the project grows.
- **Query and Filtering Needs**: Storing each topic separately in ReductStore gives you more control over how you query and filter the data, especially when you need specific metadata, like vibration data or logs, for efficient analysis.
