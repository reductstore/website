...
## Method 2: Store Rosbag Data in Time-Indexed Object Storage

![Method 2 Diagram](./img/method2.png)

Another option is to store compressed Rosbag files in a _time-indexed object storage solution_ like [**ReductStore**](/). Time-series storage works really well for ROS data because it sorts and retrieves information by time, which fits naturally with how robots process and use data. This approach also lets you add helpful labels to the files, like AI labels, sensor types, or recording locations, making it easier to manage and access your data.
...
Rosbag is still the recording unit in methods 1 and 2 above. Past a certain volume that
turns into a directory of split files that has to be merged before anything reads it
end to end. **[Continuous ingest for ROS 2](/blog/continuous-ingest-ros2)** covers what
changes when the split step goes away entirely.
