...
date: 2026-10-06
image: ./img/ros-reductstore-social.png
---
...
To install ROS2, you'll need to select a distribution that aligns with your project requirements and system compatibility. As of late 2026, three distributions are supported:

- **Jazzy Jalisco** (`jazzy`) (Release date: May 2024; End of life: May 2029)
- **Kilted Kaiju** (`kilted`) (Release date: May 2025; End of life: November 2026)
- **Humble Hawksbill** (`humble`) (Release date: May 2022; End of life: May 2027)

If you are starting today, the `jazzy` distribution is recommended for its long-term support until May 2029. Kilted Kaiju reaches end of life in November 2026, so it is not a good place to start a new project. Jazzy is the tenth release of ROS2 and supports the following platforms:
...
![Camera frames flowing into a ReductStore bucket as records keyed on header.stamp](./img/ros-reductstore-flow.svg)
...
## The header stamp is acquisition time, not arrival time

`get_timestamp` above keys every record on `msg.header.stamp`. The ROS message definition says what that field is for, in a comment on the line itself.

From `sensor_msgs/msg/Image.msg`:
...
`CompressedImage`, the message this tutorial subscribes to, carries the same line.

So the stamp is meant to be the moment the camera captured the frame, not the moment the message reached you. Note the wording. It says **should be**, not _is_. The message definition states an intent, and the driver is what actually fills the field. A driver written carelessly will hand you a clock reading taken long after the shutter closed, and nothing in ROS stops it.

![Frame arrival time varies with transfer, decode and queue, but keyed on header.stamp each record is stored 2.0 s apart, matching the camera](./img/header-stamp-timing.svg)

### Check what your own driver does

You do not have to take the driver's word for it. Measure the gap in the callback:

```python
def image_callback(self, msg: CompressedImage) -> None:
    stamp_us = self.get_timestamp(msg)
    now_us = self.get_clock().now().nanoseconds // 1000
    self.get_logger().info(f"delay {(now_us - stamp_us) / 1000:.1f} ms")
```

Three readings tell you what you have.

A small delay that stays roughly steady means the driver is stamping at acquisition and the machine is keeping up. A delay that swings with load means the driver is stamping at acquisition and you are watching the transport, which is exactly what you want kept out of your timestamps.

### One clock for every sensor

ReductStore timestamps each record with the timestamp you provide, and each modality is its own entry. The camera, the lidar and the IMU are separate entries, and a query for a moment returns whatever each of them recorded at that moment.

That only works if every entry agrees on what the time means.
...
- Key records on `msg.header.stamp`, which the message definition defines as acquisition time, rather than on the time the callback happened to run.
