...
ROS 2 is widely used for building robotic systems with sensors like cameras. While it's great for communication, it doesn't handle storage of large image data streams.
...
To install ROS 2, you'll need to select a distribution that aligns with your project requirements and system compatibility. As of May 2025, two distributions are currently supported:
...
If you are starting today, the `jazzy` distribution is recommended for its long-term support until May 2029. This is the thenth release of ROS 2 and supports the following platforms:
...
In this example `ImageListener` is a subclass of `Node`, which is part of ROS 2's client library (`rclpy`). It sets up a subscription to listen for incoming images from the `/image_raw/compressed` topic. When an image message is received by the node via `self.subscription`, it triggers `image_callback`.
...
To test the image storage system, you can run the ROS 2 node and publish image messages to the `/image_raw/compressed` topic.
...
Replace `<ros2-distro>` with the ROS 2 distribution you are using, such as `jazzy` or `iron`.

Our custom ROS 2 node directly stores the binary data from the `ImageCompressed` message to ReductStore. This means that we can control the format of the images we store by configuring the `usb_cam` package to publish images in the desired format.
...
Once the `usb_cam` node is running, we can start the custom ROS 2 node that listens to the `/image_raw/compressed` topic and stores the images in ReductStore.
