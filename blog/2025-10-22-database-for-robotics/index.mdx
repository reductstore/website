...
import BridgeDiagram from "./img/reduct-bridge-architecture.drawio.svg";
...
## Recording ROS 2 Data with Reduct Bridge

Most robotics data starts as ROS 2 topics. [**Reduct Bridge**](/docs/reduct-bridge) records those topics straight into ReductStore on the robot, one entry per topic and one timestamped record per message. From there the same data replicates to the cloud on the labels and time windows you choose.
...
This is the whole path from a running robot to a queryable, cloud replicated dataset.

Reduct Bridge is one way to get ROS 2 topics into ReductStore. And if you want full observability on top of that data, platforms like [**INSAION**](#insaion) build on ReductStore directly: their agent embeds it on the robot and handles the recording for you.
...
<img
  src={require("./img/cloud-integration.tinified.png").default}
  alt="Diagram Cloud Integration"
  style={{
    maxWidth: "640px",
    width: "100%",
    height: "auto",
...
[**INSAION**](https://www.insaion.com) is a robotics observability platform built on top of ReductStore. Instead of running as a separate service, the INSAION Agent embeds ReductStore directly on the robot as a supervised process and records every configured topic locally.

Each recording pipeline (a named group of topics such as `navigation`, `perception` or `telemetry`) writes MCAP slices into a single rolling-buffer bucket, one entry per pipeline, keyed by timestamp and labeled with the recording session. Retention is handled by ReductStore's own FIFO, HARD, and NONE policies, so an operator picks a disk quota in the UI and the edge store enforces it.

![INSAION Rolling Buffer editor](./img/insaion-pipeline-settings.png)

When an alarm fires or an operator selects a time window, INSAION runs a Surgical Cloud Sync: it queries the local ReductStore for the matching slices by time range and label, then streams those exact slices straight to S3 without routing the heavy data through its servers. And if the robot loses connectivity, it keeps recording locally and resumes uploading once the link is back.

![INSAION Incident Window editor](./img/insaion-incident-window.png)

On top of that stored data, INSAION adds fleet dashboards, per device inventory and access control, synchronized replay of camera, LiDAR, TF, and logs, and AI diagnostics. Read more in our [**ReductStore and INSAION partnership**](/blog/news/reductstore-insaion-partnership) announcement.
...
<img
  src={require("./img/observability-stack-integration.tinified.png").default}
  alt="Diagram Observability Stack Integration"
  style={{
    maxWidth: "520px",
    width: "100%",
    height: "auto",
...
I hope you found this article helpful! If you have any questions or feedback, don't hesitate to reach out on our [**ReductStore Community**](https://community.reduct.store) forum.
