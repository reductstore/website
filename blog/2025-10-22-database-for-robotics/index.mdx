...
Reduct Bridge is one way in, not the only one. [**INSAION**](#insaion) ships an agent that embeds ReductStore and records ROS 2 for you; reach for Reduct Bridge when you want to point ReductStore at your own topics directly.
...
When an alarm fires or an operator selects a time window, INSAION runs a Surgical Cloud Sync: it queries the local ReductStore for the matching slices by time range and label, then streams those exact slices straight to S3 without routing the heavy data through its servers. Recording keeps running through a dropped link, and uploads resume when connectivity returns.
