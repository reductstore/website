...
This is the whole path from a running robot to a queryable, cloud replicated dataset, with no bag files to shuffle and no reformatting in between.
...
[**INSAION**](https://www.insaion.com) is a robotics observability platform built on top of ReductStore. Instead of running as a separate service, the INSAION Agent embeds ReductStore directly on the robot as a supervised process and records every configured topic locally, at full rate.

Each recording pipeline (a named group of topics such as `perception` or `telemetry`) writes MCAP slices into a single rolling-buffer bucket, one entry per pipeline, keyed by timestamp and labeled with the recording session. Retention is handled by ReductStore's own FIFO, HARD, and NONE policies, so an operator picks a disk budget in the UI and the edge store enforces it. That is the Rolling Buffer: always the newest data that fits.

:::note[TODO: screenshot]
Add a screenshot of the INSAION Rolling Buffer editor (pipeline and quota configuration).
:::
...
:::note[TODO: screenshot]
Add a screenshot of an incident window uploading from the device to the cloud.
:::
