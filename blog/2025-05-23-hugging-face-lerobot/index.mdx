...
[**LeRobot is an open-source project by Hugging Face**](https://huggingface.co/lerobot) that makes it easy to explore the world of robotics using machine learning, even if you’ve never done anything like this before. It gives you pre-trained models, real-world data, and simple tools built with PyTorch, a popular machine learning framework. Whether you're just curious or ready to try your first robotics project, LeRobot is a great place to start.

{/* truncate */}

## What You Need to Get Started

You can run everything in simulation, right from your browser — no robot, no installations, and no powerful computer needed. We’ll be using Google Colab, a free cloud-based coding environment.

Here’s what you’ll need:

1. **Google Account:** To use Colab, you need a Google account. If you use Gmail, you already have one. If not, you can [**create a Google account**](https://accounts.google.com/signup).

2. **Hugging Face Account:** LeRobot uses models and datasets hosted on Hugging Face. To access them, you'll need to [**create a Hugging Face account**](https://huggingface.co/join).
...
Now your notebook is using a free GPU provided by Google.

#### Step 2: Clone the LeRobot Repository

Run this command in a Colab code cell to download LeRobot from GitHub. This repository is public, so you don’t need a GitHub account to download it.
...
![Video](./img/rollout_exp.png)

### Want to Try a Different Model?

You can switch from Diffusion Policy to VQ-BeT, which is trained on the same task. It’s a good way to explore how different models perform. Here’s how to do it:
...
Since we’re using Google Colab, you have access to a free GPU, which is important because training on other systems without such a GPU can be very slow. For example, in our tests on a Mac with Apple Silicon (using the MPS device), training took two hours just to complete 20 steps.

By default, the training script runs for 5000 steps, which typically takes about an hour on Colab’s GPU. If you want to try it faster, you can reduce the steps to, say, 100. This will still give you a good idea of how training works.
...
# Use your newly trained model path
...
You can always un-comment them later if you want a deeper look into the dataset structure.
...
- Explore graphs showing how the robot moved and what actions it took. Motor 0 and Motor 1, both state and action, are shown as 4 curves over time.
  These curves are plotted over time, so you can see how the robot's decisions changed from frame to frame.
...
- **Grow your projects further.** As you work more with LeRobot and collect larger amounts of data, organizing and managing that data becomes important. This can feel overwhelming at first, but understanding the basics of data management will save you time and frustration later. We recommend checking out this beginner-friendly guide, [**How to Store and Manage Robotics Data**](https://www.reduct.store/blog/store-robotic-data). It explains simple strategies for handling robot data efficiently. You don’t need to master this now, but keeping these ideas in mind will help you scale your experiments smoothly when you’re ready.
