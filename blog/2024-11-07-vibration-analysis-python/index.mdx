...
The storage side is the part that has to survive production. ReductStore is a time-indexed object store you can self host, with size quotas, conditional replication driven by record labels, and no separate database to keep in sync. [**Download ReductStore**](/download) to try the replication flow above on your own data.
