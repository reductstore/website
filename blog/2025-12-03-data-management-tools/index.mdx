...
When choosing a data management tool for robotics, it's important to look at a few key areas to make sure the tool can handle what your system requires.

- Data Type Support. Robotics systems produce different kinds of data, and each type needs different storage and processing methods.
  - Lightweight telemetry data includes things like GPS, IMU readings, joint positions, and basic system status. This data is small, quick to access, and helpful for monitoring and analytics.
  - Downsampled sensor data is reduced in resolution or frequency. It includes lower-framerate camera images or simplified lidar scans. It is useful for reviewing operations or incidents without using too much storage.
  - Full-resolution data is the complete output from sensors, such as full-rate camera frames and dense lidar scans. This takes the most storage but is necessary for deep debugging, model training, and investigating important events.

- Integration and Ecosystem. The tool should work well with other systems like ROS, MQTT, Grafana, cloud platforms, and development environments. Good integration reduces effort, speeds up workflows, and supports long-term scaling and flexibility.

- Performance and Scalability. Robotics applications need fast, reliable data processing. Any delays can affect navigation, manipulation, or safety. For instance, large files like ROS bags can overwhelm edge devices and slow them down, and uploading them to the cloud can also take time.

- Ease of Use and API Support. A simple interface makes it easier for teams to learn and work with the tool. Well-designed APIs make it easier to automate tasks, connect with other systems, and adapt the tool to different use cases.
...
To move data to the cloud, ReductStore uses **conditional replication**. You can define rules to upload only certain records: by label, topic, or event. For example, replicate all incident data, or just 1 out of 10 entries for routine monitoring.
...
Its main strength is combining flexible logging with a fast, built-in 3D viewer designed for robotics and XR applications. For large datasets, Rerun provides a **column-oriented API** to speed up ingestion and reduce memory usage. It also uses efficient internal structures to minimize allocations, and supports selective logging to reduce noise.
...
**Heex** is a data capture and review platform for robotics that focuses on collecting only key moments—like errors or specific events instead of logging everything. This reduces bandwidth and storage needs while keeping important context.
...
Each tool addresses a different part of the robotics data workflow. **ReductStore** is ideal for scalable storage and streaming across robot fleets and cloud. **Foxglove** excels at visualizing MCAP logs and ROS topics. **Rerun** offers flexible, real-time 3D inspection for custom applications. **Heex** focuses on capturing just the important moments for efficient fleet analysis.
