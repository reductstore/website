...
With the S3 backend, ReductStore batches and caches many small records on disk before pushing them to cloud object storage, reducing request counts and storage costs. For example if you have many pictures being stored at high frequency, ReductStore will group them into larger objects before uploading to S3.

For data visualization and monitoring, you can connect Grafana to visualize metrics. With this, you can create dashboards that combine sensor telemetry with system performance metrics.

Additionally, ReductStore supports Foxglove for MCAP visualization by creating shareable query links to easily share specific data segments for collaborative analysis.

ReductStore also provides extensions that allow you to filter or merge records directly on the server side. For example, you can query all the temperature sensor readings above a certain threshold within a specific time range accross multiple MCAP files, without needing to download and process the entire dataset locally.
