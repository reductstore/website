...
| :--------------------: | :----------------------------------------------------------------------------: | :-------------------------------------------------------------------------------------------: |
|     **Data Model**     |             Relational time-series database (PostgreSQL extension)             |                                  Time-indexed object storage                                  |
...
This post compares the two on design and features. For measured write and read throughput at record sizes from 1 KB to 1 MB, with the code and a repository you can run yourself, see **[TimescaleDB alternatives for multimodal data](/blog/comparisons/iot/reductstore-vs-timescaledb)**. It also covers the cases where staying on TimescaleDB is the right call.
