...
The new version of ReductStore introduces several new conditional query operators that allow you to filter data more effectively. The new operators include

**[Arithmetic Operators](/docs/conditional-query/arithmetic-operators)** performs arithmetic operations on labels in a query like `$add`, `$sub`, and `$mult`.
...
**[String Operators](/docs/conditional-query/string-operators)** performs string operations on values in a query like `$contains`, `$starts_with`, and `$ends_with`.
...
**[Miscellaneous Operators](/docs/conditional-query/miscellaneous-operators)** provides additional functionality like `$exists`, `$cast`, and `$ref`.
...
You can try it right now with our **[demo server](https://play.reduct.store/)** (API token: **reductstore**) or you can try it with our **[SaaS offer](https://cloud.reduct.store/signup)**.
