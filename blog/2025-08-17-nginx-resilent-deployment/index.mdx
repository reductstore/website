...
The **ingress** nodes may have limited storage capacity, while they need only to handle writes and replicate data to the **egress** nodes.
It can use high-rate storage like NVMe SSDs or even RAM disks, depending on your data volume.

### Egress layer

The **egress** nodes are optimized for read performance and can use larger, slower storage like HDDs or cloud object storage.
...
## Replication Keeps Data Consistent

- Ingress nodes replicate to all egress nodes
- Each egress node holds a complete dataset
- Config (buckets, replication rules, permissions) must be provisioned identically across nodes

Use `reduct-cli` or SDKs to bootstrap configs at container startup.
...
- Use **token-based auth** consistently across ingress and egress
- Add **rate limiting** for noisy clients
- Collect **NGINX and ReductStore logs/metrics**
- Schedule **hot backups** with `reduct-cli cp` and test **cold restores** regularly
- Keep a **pilot-light instance** for cross-site resilience
...
By fronting ReductStore with NGINX and separating **ingress** and **egress** layers, you get a simple but powerful foundation for **fault-tolerant IIoT and robotics workloads**.
