...
A user on the **[rosbag2 tracker](https://github.com/ros2/rosbag2/issues/2335)** reported that
merging 300 GB of split recordings required roughly 600 GB of disk capacity. The original 300 GB
...
Merging is the most visible cost, but the file boundary itself creates two more, both with the
...
None of this makes rosbag2 badly built. Both costs come from the same moment, closing one file
and opening the next. Fix either one and the boundary is still there, and so is the merge it
eventually asks for.
