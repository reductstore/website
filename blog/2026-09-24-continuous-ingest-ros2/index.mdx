...
`rosbag2_transport::bag_rewrite` underneath, writes the merged output while the inputs are still
on disk, so peak usage during a merge is the sum of both. One user on the
