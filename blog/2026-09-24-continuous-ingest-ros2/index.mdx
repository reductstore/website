...
record` cap each bag, not the total. Recording continues until the disk is full, and on an
embedded robot a full disk takes the system logs and the DDS shared memory files with it, so
services start failing for reasons that have nothing to do with recording.

**CPU use grows as the recording gets longer.** Every split writes a new metadata header, and
that metadata describes all the files recorded so far. The more files already on disk, the more
each new split costs, so a long session gets steadily more expensive rather than staying flat.

None of this makes rosbag2 badly built. Both costs come from the same moment, closing one file
and opening the next. Fix either one and files are still being closed, so the merge is still
waiting at the end.
