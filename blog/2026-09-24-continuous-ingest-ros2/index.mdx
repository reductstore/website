...
Plenty of tooling still wants one. `ros2 bag play` wants a directory with metadata in it. Older
Foxglove builds want a single file, since some versions cannot open several mcap files at once,
which is exactly why merging is tempting in the first place. Anything you have scripted around
**[loading bag files for analysis](/blog/boston-dynamic-example)** expects a path, not a bucket.
...
The argument here starts when recording becomes continuous: a robot running for hours, every day,
producing data somebody will want to look at weeks later.
...
The split exists because a file has to close. The merge exists because the split did. Making the
split cheaper, which the rosbag2 maintainers are doing, does not make the merge unnecessary.

Recording each topic as its own entry removes the step instead. Nothing is being closed during
recording, so there is nothing to reassemble afterwards, and the bag goes back to being something
you export when a tool asks for one.

If you are writing a script to work out which bags overlap the window you care about, that script
