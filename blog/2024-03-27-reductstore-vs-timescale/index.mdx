...
The second problem usually leads teams to use a combination of a bucket and a table. However, we recommend checking the third option first. This is because the metadata table is often where the complexity of synchronisation, retention and consistency accumulates.

The same split shows up against **[MongoDB](/blog/comparisons/iot/reductstore-vs-mongodb)** and against **[InfluxDB and MinIO together](/blog/comparison/iot/reductstore-benchmark)**.
...
A time series database indexes by timestamp. That is what makes a range query cheap, and it is why TimescaleDB is a reasonable first thing to try. The cost is that the payload has to stay small.
...
The first decision is whether to move the payload out. The second is what you can do with it once it is out, and this is where a time-indexed store differs from a simple bucket.
...
TimescaleDB is the right choice when your records are structured rows you aggregate and join in SQL, and the payloads stay small enough to sit inline, under roughly 2 KB. Push past that threshold with opaque, high entropy blobs and the cost we measured above shows up: TOAST chunking and an extra assembly step on every read.
