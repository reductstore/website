...
The benchmark's own client path pays a second cost on top of that. It writes through `execute_values`, which builds the entire `INSERT` statement in memory before any of it goes on the wire, so a 1 MB record is a 1 MB string in the client process before it is a 1 MB value in the server. That is a property of the batching helper this benchmark uses, not a limit of PostgreSQL itself: `COPY` can stream a value to the server without building it in memory first.

TimescaleDB's columnar compression does not recover it either. Compression works by finding similarity across values, and an already compressed, high entropy binary payload such as a JPEG has little of that left to exploit.
