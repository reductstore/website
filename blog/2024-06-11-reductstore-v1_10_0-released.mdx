...
You can also use it to save downsampled data for long-term storage while keeping the original data for short-term analysis. For example, if you **[store data from vibration sensors](/blog/how-to-store-vibration-sensor-data)**, you can replicate one record per hour to a long-term bucket to see drifts in the data over years.
