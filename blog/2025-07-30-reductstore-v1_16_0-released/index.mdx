...
Our goal isn't to compete with full-featured time-series databases, but to offer a lightweight solution for simple data transformation and filtering tasks—directly within ReductStore. This reduces the need to install and maintain a separate database for such operations.
...
- Extract data from MCAP files containing ROS 2 messages and convert it into JSON format for easier analysis and visualization.
- Convert raw binary data, such as images, into more accessible formats like JPEG or base64 strings.
...
We are also working on a feature that allows users to generate and share links to specific queries in ReductStore.
...
We’re developing a **[Grafana plugin](https://github.com/reductstore/reduct-grafana)** that enables users to visualize and analyze data stored in ReductStore directly within Grafana dashboards.
