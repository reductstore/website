...
<small>
  Sensor data processed and labeled by AI, stored in ReductStore, with metadata
  relayed to Kafka
</small>
...
- The volume mapping `./data:/data` creates persistent storage by mapping a local directory (`./data`) to the `/data` directory inside the container. Since ReductStore v1.19 runs as a non-root user, prepare the host directory first: `mkdir -p ./data && sudo chown -R 10001:10001 ./data`.
