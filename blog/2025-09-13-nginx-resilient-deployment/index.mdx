...
image: ./img/disaster_recovery_active_active.png
---

# Building a Resilient ReductStore Deployment with NGINX

If you’re collecting high-rate sensor or video data at the edge and need zero-downtime ingestion and fault-tolerant querying, an **[active–active ReductStore setup](/docs/guides/disaster-recovery#hot-standby-setup)** fronted by NGINX is a clean, practical pattern.
...
[**Install the `reduct-cli` tool**](/download) if you haven't already, then run the following commands to set up aliases for the ingress and egress endpoints:
