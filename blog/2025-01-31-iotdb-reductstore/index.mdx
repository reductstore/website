...
IoTDB is highly optimized for structured, schema-based time-series data with pre-defined fields and hierarchies (e.g., devices, measurements, etc.). This is espacialy good, if you have many devices of the same type, such as gasoline vehicles, and their measurements (such as speed, fuel quantity, etc.) are of the same type, storing metadata for each time series can cause problems. The metadata (such as units and measurement types) takes up a lot of storage, and managing so many records becomes complex. To solve this problem, IoTDB uses a schema template feature that allows similar devices to share the same metadata, reducing storage usage and making it easier to manage data from many devices.
