...
Time-series databases usually have retention based on time, but that is not always enough when the data volume can spike.
That can lead to out-of-disk errors, data loss, and downtime.

FIFO (first-in-first-out) quota is a better fit for this workload because it removes the oldest data when the disk budget is reached
and guarantees that the newest data is always accepted and stored.
This way, you can keep a rolling history of the most recent data without worrying about disk management

**Data replication** also needs a different model.
...
docker run --rm --network host \
  ...
  reduct/bridge:latest-iot reduct-bridge /etc/reduct-bridge/config.toml
```
...
I hope you found this article helpful. If you have any questions or feedback, don't hesitate to use the [**ReductStore Community**](https://community.reduct.store) forum.
