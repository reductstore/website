...
async def main():
    async with Client("http://127.0.0.1:8383", api_token="my-token") as client:
        ...
        async for record in bucket.query(
            ...
            filename = output_dir / f"frame-{record.timestamp}.jpg"
            with open(filename, "wb") as output:
                output.write(frame)
            print(filename, record.labels)
...
The query selects records by time range and labels, then saves each binary body as a `.jpg` file without decoding or re-encoding it.
For vibration buffers, use the `mqtt/vibration` entry and write the bytes to the format expected by your analysis pipeline.
