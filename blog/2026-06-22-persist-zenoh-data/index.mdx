...
_A publisher writes via `put(key, value)` to zenohd's storage manager, which persists into a backend volume. A client's `get` is routed to the storage. By default, queries against storage-managed volumes return the latest stored value for each matching key. The InfluxDB backend additionally supports historical queries through the `_time` selector._
...
Unlike the InfluxDB backend, the memory, RocksDB, filesystem, and S3 backends do not expose a documented time-range query mechanism comparable to `_time`. They are designed around latest-value retrieval rather than historical time-range queries.
