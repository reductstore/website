...
## How teams currently solve this (and why it's painful)

Zenoh ships official storage backends too: a built-in `memory` volume, plus S3, InfluxDB, RocksDB, and filesystem via the `zenoh-plugin-storage-manager`. Point one at a set of key expressions and, per Zenoh's own docs, other nodes can then "query the most recent values associated with these keys."

```svgbob
+-----------+     +-----------+     +-------------------+
| Publisher | --> |  zenohd   | --> | Volume             |
| robot/*   |     | (storage  |     | (S3, InfluxDB,     |
+-----------+     |  manager) |     |  RocksDB, FS, mem) |
                   +-----------+     +-------------------+
                         ^
                         |
                   +-----------+
...
_Zenoh's storage manager plugin aligns the latest value per key into a backend volume._

That's a key-value store: it keeps the latest value per key. Camera frames and LiDAR scans aren't "the current value" of something, they're a stream where every sample matters. Querying through the storage manager gets you the latest value, not "every frame from robot alpha between 2pm and 2:10pm where status was warn." And retention is whatever the backend offers, not a size limit you can set for a robot's disk.

So teams end up building on top. Two paths are common:

**Write a custom Zenoh storage backend.** Zenoh's storage manager API lets you plug in your own backend, but you're now maintaining a Rust component that has to keep up with Zenoh's release cadence, handle backpressure, and survive reconnects on flaky networks. For a small team, this is weeks of work for something that isn't your product.

**Write a custom subscriber that forwards to your existing storage over HTTP.** Zenoh's [**REST plugin**](https://zenoh.io/docs/manual/plugin-http/) only exposes Zenoh's own key/value space over HTTP, it doesn't forward to an external database. So you write an app that subscribes to the key expressions, pushes each sample to Postgres or InfluxDB yourself, and builds the time-range and label queries on top. It works for low-rate telemetry, but it's another process to run, and at 10 Hz LiDAR or 2 Hz camera frames across a fleet, that extra hop becomes a bottleneck and a single point of failure.

Either way, you're building and maintaining a component just to get time-range queries, label filtering, and size-based retention for continuous robotics data.

## A storage backend that speaks Zenoh natively

Starting with version 1.19, [**ReductStore**](/) (an open source time series object store built for robotics and industrial data) ships a native Zenoh API. It keeps every sample, not just the latest, indexed by time and label, so the queries above just work. A ReductStore instance joins your Zenoh network as a regular peer: it registers a **subscriber** for writes and a **queryable** for reads, talking directly to the storage engine. No bridge process, no extra protocol, no custom Rust backend to maintain.

```svgbob
+-----------+                +----------------+                +--------------+
| Publisher | -- pub ------> |                | <-- get ------ | Query Client |
| robot/*   |                |  Zenoh network |                +--------------+
+-----------+                |                |
                              +----------------+
                                     ^   |
...
_A publisher sends data over Zenoh; ReductStore joins as a peer with a subscriber for writes and a queryable for reads; a query client reads time ranges back out._
