...
Zenoh ships official storage backends: memory, S3, InfluxDB, RocksDB, and filesystem via the `zenoh-plugin-storage-manager`. By default, queries return the latest stored value for each matching key. The InfluxDB backend goes further with a `_time` selector for historical queries:

```
robot/alpha/**?_time=[now(-5d)..now()]
```

That works for structured numeric telemetry. The problem is that InfluxDB stores binary payloads as base64-encoded strings, wildcard queries only work with v1.x, and sample attachments don't map to InfluxDB tags. The memory, RocksDB, filesystem, and S3 backends don't have a time-range query mechanism at all.

What was missing was a backend built for binary time-series data: one that stores every sample as-is, indexes it by time, and fits Zenoh's data model without a translation layer. That's what led to building a native Zenoh API in ReductStore.

## A storage backend that speaks Zenoh natively

Starting with version 1.19, [**ReductStore**](/) (an open source time-indexed object store built for robotics and edge data) ships a native Zenoh API. It keeps every sample, not just the latest, indexed by time, so the queries above just work. A ReductStore instance joins your Zenoh network as a regular peer: it registers a **subscriber** for writes and a **queryable** for reads, talking directly to the storage engine. No bridge process, no extra protocol, no custom backend to maintain.

```svgbob
     +-------------------------+     +-------------------------+
     |      "robot-alpha"      |     |      "robot-beta"       |
     |    "robot/alpha/**"     |     |    "robot/beta/**"      |
     +-------------------------+     +-------------------------+
                |                                |
               "pub"                            "pub"
        ...
        +-------------+---------------------------+----------+
                      ^                           |
                      |                           |
                 "sub/query"        "get(...?start=…;stop=…)"
                      |                           |
                      v                           v
        +-------------+---------------+ +---------+----------+
        |         ReductStore         | |    "remote app"    |
        |     "fleet (1 GB FIFO)"     | +--------------------+
        +-----------------------------+
                       |
                  "HTTP 8383"
...
services:
  ...
  reductstore:
    ...
    environment:
      ...
      RS_BUCKET_0_NAME: "fleet"
      RS_BUCKET_0_QUOTA_TYPE: "FIFO"
      RS_BUCKET_0_QUOTA_SIZE: "1 GB"
...
`RS_ZENOH_SUB_KEYEXPRS` and `RS_ZENOH_QUERY_KEYEXPRS` are key expressions, wildcard patterns like `robot/**`, that define which keys ReductStore subscribes to and answers queries for. The `fleet` bucket is created automatically on startup, and the quota settings make it a rolling store that keeps the most recent 1 GB of data.
...
For replay, debugging, or building a training set, you don't have to stay on the Zenoh API. The same data is available over HTTP via the `reduct-py` SDK (or the JS, Go, Rust, and C++ SDKs):
...
async def main():
    async with Client("http://localhost:8383", api_token="my-token") as client:
        bucket = await client.get_bucket("fleet")
...
This is more robust at the edge than a time-based "keep the last 24 hours" policy: if a robot goes offline for a few days, a time-based rule would silently wipe everything by the time it reconnects. A size-based policy just keeps the most recent 1 GB, however long that takes to fill.
