...
```
robot/alpha/**?_time=[now(-30d)..now()]
```

That works for structured numeric telemetry. The problem is that InfluxDB stores binary payloads as base64-encoded strings, wildcard queries only work with v1.x, and sample attachments don't map to InfluxDB tags. The memory, RocksDB, filesystem, and S3 backends do not expose a documented time-range query mechanism comparable to InfluxDB's `_time` selector.

What was missing was a backend built for binary time-series data: one that stores every sample as-is, indexes it by time and label, and fits Zenoh's data model without a translation layer. That's what led to building a native Zenoh API in ReductStore.
