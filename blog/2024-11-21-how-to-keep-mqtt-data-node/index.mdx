---
title: "Node.js MQTT Client That Stores Every Message"
description: "A Node.js MQTT client that subscribes to every topic on a Mosquitto broker and writes each message with its timestamp, so the history is queryable later."
authors: alexey
