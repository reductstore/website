...
- **???Limited Analysis**. RViz and RViz 2 primarily serve visualization purposes and lack detailed message inspection, conditional logging, or advanced playback controls like pause, step, or speed adjustment. External tools such as rqt_bag or ROS command-line utilities are needed to fill these gaps. Additionally, RViz does not consistently warn about invalid data (e.g., NaNs or infinities), which can cause missing or inaccurate visuals. The tools are not designed for deep offline data analysis.
...
- **???Programmable Data Access and Web Integration**. Beyond its interactive Viewer, Rerun provides structured data access through Apache Arrow, enabling programmatic analysis in tools like Pandas or Jupyter. This facilitates seamless integration of visual and quantitative workflows.
