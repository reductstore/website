...
[ReductStore](https://wwww.reduct.store/) is designed to address these issues. It features a robust FIFO quota, an HTTP API for data querying and batching over time intervals, and arranges objects (or records) into blocks for optimal disk usage and search efficiency.

Both [Minio](https://min.io/) and [ReductStore](https://wwww.reduct.store/) offer Python SDKs. These can be used to implement read and write operations and to compare performance. As both databases utilize the HTTP protocol, we will use blobs of varying sizes to estimate the impact of HTTP overhead and to see how ReductStore optimizes this aspect.

## Read/Write Data With Minio

Let's begin with Minio and its Python SDK. For benchmarks, we'll create two functions: one to write and the other to read `BLOB_COUNT` blobs of `BLOB_SIZE`.
...
With [ReductStore](https://wwww.reduct.store/), getting data by time intervals is much easier, as it provides a special API for this purpose:
