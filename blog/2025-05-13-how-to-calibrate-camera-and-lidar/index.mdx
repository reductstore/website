...
## [How MetriCal Works](https://docs.tangramvision.com/metrical/intro/)
...
Once the dataset and configuration files are ready, MetriCal’s `init mode` analyzes sensor observations to infer a raw input plex — a representation of the physical system that you are calibrating. This plex is the starting point for all further calibration steps.
...
MetriCal simplifies multimodal sensor calibration by providing a fully scriptable, CLI-based workflow with detailed diagnostics and seamless ROS integration. Designed for reproducibility and automation, it supports everything from rapid prototyping to production-scale deployment, ensuring accurate and consistent calibration for sensor-driven robotic systems.
