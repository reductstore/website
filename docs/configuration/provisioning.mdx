...
| `RS_REPLICATION_<ID>_DST_PREFIX`  |         | Prefix added to destination entry names. For example, `robot-1` writes `/camera/image` as `robot-1/camera/image`.                   |
| `RS_REPLICATION_<ID>_ENTRIES`     |         | Comma-separated entry names or patterns to replicate. If empty, all entries are replicated. Supports `*`, `**`, and `!` exclusions. |
...
| `RS_REPLICATION_<ID>_COMPRESSION` | none    | Replication transfer compression. Accepted values: `none`, `gzip`, `zstd` (case-insensitive).                                       |
