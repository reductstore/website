...
Batch protocols send record metadata in HTTP headers. If a client uses HTTP/1.1, the `RS_IO_BATCH_MAX_METADATA_SIZE` and `RS_IO_BATCH_MAX_RECORDS` settings limit the size of the metadata headers in the HTTP request.
This means that the client can't send more records in a single batch than specified by `RS_IO_BATCH_MAX_RECORDS`, and the request metadata headers can't exceed `RS_IO_BATCH_MAX_METADATA_SIZE`.

Read more about batching in the **[I/O API Batch Protocol v2](../http-api/io-api#batch-protocol-v2)** and the legacy **[Entry API Batch Protocol](../http-api/entry-api#batch-protocol)**.
...
- `lifecycle_run` records are written to `$system` under the `lifecycle/<instance-name>/<policy-name>` entry path.
- Lifecycle run payloads always include `processed_records`, `processed_blocks`, and `caught_up`. When available, they also include `last_processed_ts`, which ReductStore uses to resume bounded lifecycle progress between runs.
