...
| `RS_IO_MAX_READERS_IN_FLIGHT`   |         | Optional maximum number of storage read operations that can run at the same time. Positive integers enable the limit; unset or `0` disables it.    |
...
The in-flight reader and writer limits are optional and disabled by default. They are useful for exposed or multi-tenant deployments where you need to bound concurrent storage I/O and protect disk or remote-backend resources from overload. If all reader or writer slots are busy, ReductStore waits up to `RS_IO_OPERATION_TIMEOUT`; if no slot becomes available, the request fails with **429 Too Many Requests**.
