...
Read-only replicas keep the existing behavior: `RS_API_TOKEN` is optional, while `RS_INIT_API_TOKEN` and `RS_DISABLE_AUTH` do not apply. See the **[Access Control guide](../guides/access-control)** for details.
