...
The arithmetic operators are used to perform mathematical operations on the values of labels in a query and use them to filter records.
...
Additional rules:
_ Boolean operands are cast to integers before the operation.
_ If the list of operands is empty, the operator returns 1.
...
Additional rules:
_ All operands are cast to floats before the operation.
_ If the second operand is 0, the operator returns an error.
...
Additional rules:
_ All operands are cast to integers before the operation.
_ If the second operand is 0, the operator returns an error.
...
Additional rules:
_ Boolean operands are cast to integers before the operation.
_ The remainder can be negative. \* If the second operand is 0, the operator returns an error.
