...
The querying stops when the batch size is reached but put the current record in the batch, even if it exceeds the limit.
...
The querying stops when the metadata size in the batch reaches the specified limit but put the current record in the batch, even if it exceeds the limit.
...
This directive overrides the server's `RS_IO_RECORD_TIMEOUT` setting. See the [**server configuration documentation**](../configuration.mdx#io-settings) for more details.
