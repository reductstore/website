...
| --------- | ------------------- | ----------------------------------------------------------------------------- |
| `$each_n` | v1.16               | Keeps each N-th record after previous condition                               |
| `$each_t` | v1.16               | Keeps a record after the previous condition once in time interval             |
| `$gate`   | v1.20               | Opens a fixed time window on `false -> true`, then forces `false` until reset |
| `$limit`  | v1.16               | Limits the number of records to be processed in the query                     |
...
The `$gate` operator opens a time-bounded gate when the input condition changes from `false` to `true`.
While the gate is open, output mirrors the input condition. Once the time window expires, output is forced to `false` until the input is reset to `false` and a new `false -> true` edge occurs.
