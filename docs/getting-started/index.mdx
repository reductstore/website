...
:::note
Since v1.19, the Docker container runs as a non-root user. Create the host folder in advance and make sure it is writable by UID/GID `10001:10001` before mounting it.
:::

The database will be available at `http://127.0.0.1:8383` and stores data in the `./data` directory. The `RS_API_TOKEN` environment variable configures the initial full-access API token. You may check if the database is working with an authenticated HTTP request:

```bash
curl --header "Authorization: Bearer my-token" http://127.0.0.1:8383/api/v1/info
```
...
If you familiar with Rust, you can build the database from the source code. The source code is available on [**GitHub**](https://github.com/reductstore/reductstore) or
use Cargo to install the database.
...
You may need to install a newer Rust version. Read the **[official Rust installation guide](https://www.rust-lang.org/tools/install)** for more information.
...
ReductStore will be available at http://127.0.0.1:8383 and store data in the `/var/snap/reductstore/common/data` directory.
