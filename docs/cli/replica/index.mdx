...
- `<REPLICATION_PATH>`: Path to a resource to use (e.g. SERVER\_ALIAS/&lt;RESOURCE\_NAME&gt; or http://token@localhost:8383/RESOURCE\_NAME)
- `<SOURCE_BUCKET_NAME>`: Source bucket on the replicated instance
- `<DEST_BUCKET_PATH>`: Path to a resource to use (e.g. SERVER\_ALIAS/&lt;RESOURCE\_NAME&gt; or http://token@localhost:8383/RESOURCE\_NAME)
