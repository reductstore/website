...
ReductSelect can use SQL to select, filter, aggregate, and reorder rows. When
`sql` is omitted, it skips SQL processing and can pass through, batch, or
convert the input depending on the other options.
...
```python
{
    ...
    # Optional SQL query to run against the current record
    "sql": "string",
    # Optional output controls
    "export": {
        # Output format: "csv", "json", or "parquet"
        "format": "string",
        # Batching: group input or SQL result rows into output records with at most this many rows each. Must be greater than 0.
        "rows": "number",
        # Batching: group input or SQL result rows into output records by source timestamp span, as microseconds or a duration string such as "1s", "1.5s", or "1m".
...
If `sql` is not specified, ReductSelect skips SQL parsing and execution. When
the query also omits batching, computed labels, Protobuf field extraction, and
output format conversion, the original payload and metadata are returned
unchanged without parsing the data.

When `sql` is omitted but batching, computed labels, Protobuf field extraction,
or a different `export.format` is requested, ReductSelect parses only the data
needed for that operation and bypasses the SQL engine. For example, a JSON-to-
Parquet conversion does not create or execute a DataFusion SQL session.
...
| `protobuf` | object | No        | Inferred from content type | Selects Protobuf input parsing. Cannot be used together with `csv`, `json`, or `parquet`.                             |
| `sql`      | string | No        | Not set                    | SQL query executed against the current input record. When omitted, SQL parsing and execution are skipped.             |
...
| `rows`     | number           | No        | All rows        | Batches input or SQL result rows into output records with at most this many rows per output record. Must be greater than 0.                        |
| `duration` | number or string | No        | Not set         | Batches input or SQL result rows into output records by source timestamp span, as microseconds or a duration string such as `1s`, `1.5s`, or `1m`. |

:::tip Batch before SQL in a pipeline
If one pipeline stage only needs to batch rows for a later SQL stage, put the
batching-only `select` step first and omit both `sql` and `export.format` from
that step:
...
```json
{
  "#ext": [
    ...
    {
      "select": {
        "sql": "SELECT AVG(value) AS value FROM ENTRY()"
      }
    }
  ]
}
...
This is considerably faster than running a projection query while batching.
The first step avoids SQL parsing and processing as well as format conversion,
so it performs only the batching needed by the next step.
:::
