...
```python
{
    "ext": {
        "ros": {
            ...
            "transform": {
                "include": ["/topic-.*"], # List of topics to include
                "exclude": ["/topic-b", "/ext-.*"], # List of topics to exclude
                "duration": "1m", # Max episode duration
...
| `size`      | `string`   | No        | Maximum file size per episode (e.g., `100MB`, `1GB`).                                          |

:::info
If neither `duration` nor `size` is provided, the transform will output the original MCAP files with only topic filtering applied.  
If both `duration` and `size` are provided, whichever limit is reached first triggers a new episode.
:::
