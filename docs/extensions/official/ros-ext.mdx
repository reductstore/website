...
- Transformed `MCAP` files or `JSON` representations of ROS 2 messages
...
import RosExtractFilteredTopicPy from "!!raw-loader!../../examples/py/src/ext_ros_mcap_extract_filtered.py";

<Tabs>
  <TabItem value="Python">
    <CodeBlock language="python">{RosExtractFilteredTopicPy}</CodeBlock>
  </TabItem>
</Tabs>
...
- Each record corresponds to one ROS 2 message and includes:
- The decoded JSON payload, e.g., `{"data":"hello"}`
- The output is filtered, as the second entry did not have a matching `data` value.
- Message metadata as labels, including:
  - `topic`: `/test`
  - `schema`: `std_msgs/String`
  - `encoding`: `cdr`
