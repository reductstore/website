...
The ReductStore API allows you to delete specific records from an entry, the entire entry or the entire bucket.
You can do it data per bucket or per entry by using the Client SDKs, Reduct CLI or HTTP API.

### Deleting Entry or Bucket

The following examples demonstrate how to delete the entire entry or the entire bucket:
...
You can remove a sole record from an entry or a batch of records by specifying their timestamps.
...
You can delete records in a specific time range by specifying the start and end timestamps and use query parameters to filter the records:
