...
An entry can be renamed using the SDKs, CLI client, Web Console, or REST API. A client must have full access permission to a bucket to rename its entries.
...
<Tabs>
  ...
  <TabItem value="Web Console">
    Steps to rename an entry using the Web Console: 1. Open the Web Console at
    [http://127.0.0.1:8383](http://127.0.0.1:8383) in your browser. 2. Enter the
    API token if the authorization is enabled. 3. Select the bucket that
    contains the entry you want to rename ![Browse buckets with Web
    Console](./img/entry_rename_select_bucket.webp) 4. You will see a list of
    all the entries in the bucket 5. Click on the edit icon in the entry row:
    ![Edit entry with Web Console](./img/entry_rename_edit.webp) 6. Rename the
    entry and click on the **Save** button: ![Save entry name with Web
    Console](./img/entry_rename_save.webp)
  </TabItem>
  <TabItem value="Python">
