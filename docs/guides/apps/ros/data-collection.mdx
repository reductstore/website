---
title: Collecting ROS 2 data with ReductBridge
...
<head>
  <link
    rel="canonical"
    href="https://www.reduct.store/docs/guides/apps/ros/data-collection"
  />
</head>

# Collecting ROS 2 data with ReductBridge

This guide explains how to configure ReductBridge to collect ROS 2 messages as
time-indexed ReductStore records. The example preserves serialized CDR payloads
and ROS schemas, assigns timestamps and labels, and writes the records to a
local ReductStore instance.

## Data flow

ReductBridge moves each ROS message through the input, pipeline, and remote
stages before writing it to the local ReductStore bucket.
...
ReductBridge is available as Docker images, prebuilt Linux binaries, and Snap
packages for Ubuntu 22.04 and later. You can also build it from source. See the
**[ReductBridge documentation](../../../reduct-bridge/index.mdx#installation)**
for all installation options.

The Docker example requires Docker with Docker Compose and host networking. On
Docker Desktop, enable host networking before starting the stack. See Docker's
...
- `ROS_HOME: /tmp/ros` gives ROS 2 a writable directory for logs and other
  runtime files. These files do not need to persist after the container stops.
- `FASTDDS_BUILTIN_TRANSPORTS: UDPv4` makes Fast DDS use UDP/IPv4 instead of
  shared memory, avoiding shared-memory transport errors between isolated
...
ReductBridge with its ReductStore and ROS publisher dependencies:
...
The complete configuration is available as the
**[bridge.toml example](https://github.com/reductstore/website/blob/main/docs/guides/apps/ros/example/bridge.toml)**:
...
Pipeline label rules can enrich records from one topic with context extracted
from another. The pipeline's label configuration contains two propagation rules
and one static rule:

```toml file=./example/bridge.toml start=pipeline-labels-start end=pipeline-labels-end
...
```toml file=./example/bridge.toml start=remote-start end=remote-end
...
The example uses default batch settings. See the
**[ReductStore remote reference](../../../reduct-bridge/remote/reduct/index.mdx)**
to tune them.
