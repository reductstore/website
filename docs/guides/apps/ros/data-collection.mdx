...
This guide explains how ReductBridge uses the `bridge.toml` configuration from
the **[ROS applications guide](./index.mdx#data-collection)** to turn ROS 2
topics into time-indexed ReductStore records.

Data flows from ROS 2 topic subscriptions through a pipeline that enriches and
routes records to a ReductStore destination.
...
ReductBridge is available as a Docker image and prebuilt binaries for Linux,
...
The service runs the ROS 2 Jazzy image of ReductBridge on the host network so it
can discover the example ROS publisher. It mounts `bridge.toml` at
