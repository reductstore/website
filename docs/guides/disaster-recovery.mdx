...
| Cold Backup via File System                | Coping data as files                                                                           | minutes/hours | ✅                | ✅               | ❌              | ✅                  |
| Hot Backup via CLI                        | Coping data with ReductStore CLI to a buckup instance                                          | zero          | ✅                | ✅               | ❌              | ✅                  |
...
The **hot backup** strategy uses the ReductStore CLI to copy data from a live instance to a secondary (backup) instance.
...
You will need to provision the backup instance with the same configuration using CLI, SDK, or infrastructure-as-code tools.
...
Use tools such as **Terraform**, **Ansible**, **ReductStore CLI**, or SDKs to automate and enforce configuration consistency.
