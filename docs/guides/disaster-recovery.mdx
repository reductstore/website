...
Replicas serve **reads only** and periodically refresh bucket metadata and indexes from the storage backend, so newly written data is not visible immediately. You can tune this behavior with `RS_ENGINE_REPLICA_UPDATE_INTERVAL` on replicas and `RS_ENGINE_COMPACTION_INTERVAL` on the writer, which controls how often WALs are compacted into blocks and synced to the backend (in addition to syncing when a block is finished).
...
### Example: Docker Compose (shared file system)
...
For a full walkthrough using S3, see **[S3 Integration](../integrations/s3#read-only-replica-on-s3)**.
