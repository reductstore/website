...
:::info
Due to the limitation described above, the size of the record must be known in advance even though the record is sent in chunks.
:::
...
In ReductStore, you can add labels to the record to annotate it. Labels are key-value pairs that can be used for filtering and querying data.
Moreover, you can add a content type to the record to specify the type of data. It maybe useful for data interpretation on the client side.
...
In case of little size records, it is recommended to use the batch ingestion mode. In this case, a client application sends a batch of records in a single request to reduce the overhead of the HTTP protocol.
You can also check the status of the batch ingestion and get the list of failed records.
