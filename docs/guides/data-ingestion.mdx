...
description: Complete guide on how to ingest data into ReductStore using the  SDKs or HTTP API.
---
...
In this section, you can learn how to implement typical data ingestion cases using the ReductStore SDKs or HTTP API. Please note that all examples are tailored for a local ReductStore instance accessible at **http://127.0.0.1:8383** using the API token 'my-token'.
...
The simplest way to write data into ReductStore is by sending a single record containing one value. In this case, the record is transmitted as a binary object in the body of the POST request.
...
For large records, using the streaming approach is advisable. Here, a client application sends a record in chunks, eliminating the need to load the entire record into memory. This approach is particularly beneficial for real-time data ingestion when data is received from a stream and needs to be sent to ReductStore without buffering.
...
In ReductStore, you can annotate records by adding labels. Labels, which are key-value pairs, can be used to filter and query data. Furthermore, you can specify the data type by adding a content type to the record, which can aid in data interpretation on the client side.
...
For smaller-sized records, it is recommended to use batch ingestion mode. This mode allows a client application to send a batch of records in a single request, reducing HTTP protocol overhead.
