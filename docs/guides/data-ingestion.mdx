...
<Tabs>
  ...
  <TabItem value="Web Console">
    ...
    7. In the upload file modal, select select the entry nane or create a new one and select the file to upload:
    ![Change bucket settings with Web Console](./img/buckets_upload_file.webp)
    8. Click on the **Upload File** button to upload the file.
    9. Optionally, you can set timestamp and content type for the record.
      If the timestamp is not set, the current time will be used. If the content type is not set, the Web Console will try to detect it automatically.
...
<Tabs>
  ...
  <TabItem value="Web Console">
    ...
    7. In the upload file modal, select select the entry nane or create a new one and select the file to upload:
    ![Change bucket settings with Web Console](./img/buckets_upload_file.webp)
    8. Click on the **Add Label** button to add a label and its value to the record. You can add multiple labels by clicking the button multiple times.
