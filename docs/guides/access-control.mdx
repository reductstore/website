...
### System buckets and audit access

Buckets whose names start with `$` are treated as **system buckets** (for example, `$audit`).
For system buckets, ReductStore requires an **exact bucket match** in token permissions:

- wildcard permissions like `*` or `prod-*` do **not** grant access to `$...` buckets;
- to access the audit bucket, include `$audit` explicitly in `read` and/or `write` permissions;
- tokens with `full_access: true` can still access system buckets.
