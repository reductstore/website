...
ReductStore uses token-based authentication to secure the API. A client must send a valid token in the `Authorization' header in order to access the API.
...
- `read`: List of buckets allowed to read data from entries.
- `write`: List of buckets allowed to write data to entries or delete them.

The table below shows the list of operations that can be performed with each permission:
