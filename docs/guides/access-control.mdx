...
<Tabs>
    ...
    <TabItem value="Web Console">
        ...
        6. Copy the token value and save it in a secure place.<br /><img
        src={
...
<Tabs>
    ...
    <TabItem value="Web Console">
        ...
        6. Confirm the action by typing the token name and clicking on the **"Remove"** button.<br /><img
        src={
...
Audit is controlled by `RS_AUDIT_ENABLED` and is disabled by default.
When `RS_AUDIT_ENABLED=true`, interactions are stored in aggregated form in the `$audit` bucket per token and available via the standard API.

You can cap audit storage with `RS_AUDIT_QUOTA_SIZE`; when set, `$audit` uses FIFO rotation (oldest records are removed first).

Read-only replicas do not have write access, so they forward the audit log to the primary/secondary instance.
To enable auditing on a read-only replica, set `RS_PRIMARY_URL` (and optionally `RS_SECONDARY_URL` for failover).
The `RS_API_TOKEN` used by the replica must also have write access to the `$audit` bucket on the destination instance.

For HTTPS forwarding from replicas, use:

- `RS_AUDIT_REMOTE_VERIFY_SSL` (default `true`)
- `RS_AUDIT_REMOTE_CA_PATH` (custom CA bundle)
- `RS_AUDIT_REMOTE_TIMEOUT` (request timeout in seconds)
