...
Audit logging is controlled by the `RS_AUDIT_ENABLED` environment variable and is **disabled by default**.
When `RS_AUDIT_ENABLED=true`, interactions are stored in an aggregated form in the `$audit` bucket, scoped per API token.
The data can be accessed via the standard API.
