---
title: Deployment as Azure Virtual Machine
...
# Deployment as Azure Virtual Machine
...
It uses memory caching to optimise read and write operations by reducing the number of requests to the Azure Storage Account.
...
Additionally, you can configure and provision ReductStore's resources, read [Configuration](./docs/configuration.mdx) for more information.
...
<Tabs>
    <TabItem value="Azure CLI">
        ...
        ```bash
        az vm image list --publisher reductstorellc1689939980623 -o table --all
        ```
        ...
        ```bash
        az vm image terms accept --publisher reductstorellc1689939980623 -f reductstore-server --plan [PLAN_NAME]
        ```
        ...
        --name myVM \
        --image reductstorellc1689939980623:reductstore-server:[PLAN_NAME]:[VERSION] \
        --size Standard_A2_v2 \
    ...
    <TabItem value="Azure Portal">
        ...
        2. Search for "ReductStore" and select the "ReductStore Time-Series Blob Storage in the Cloud" offering.
