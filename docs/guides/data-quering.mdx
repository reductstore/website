...
While it is possible to retrieve a record by its timestamp, this method is less optimal than querying by a time range due to the lack of batching. However, this approach can be used when timestamps serve as identifiers for non-time series records, such as models or configurations.
