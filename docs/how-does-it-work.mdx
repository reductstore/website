...
- **No Built-in Aggregation.** ReductStore doesn't provide aggregation functionality out of the box.
  You can't calculate averages, sums or other statistics on data stored in the storage engine directly.
  However, you can use extensions for well-known data formats to add aggregation support. For example, you can use [**Official Extensions**](./extensions/index.mdx) to extract data more efficiently and aggregate it.
...
Entry names can be hierarchical paths such as `robot/front/camera/image`, where each part is a segment of the full entry name.
...
If you need metadata that is common for all records in an entry (for example, schema or calibration info), you can store it as **attachments** in a system entry `<entry_name>/$meta` instead of duplicating it in record labels.

Attachments are kept in a hidden system entry (not shown in regular user entry lists), and they can be replicated together with data from the parent entry.
...
ReductStore supports resilient deployment patterns for both write and read workloads:

- **Primary-secondary hot standby** for failure-tolerant ingestion.  
  A primary node handles writes while a secondary node stays in sync and can take over quickly if the primary fails.
- **Read-only replication** for scalable and resilient data access.  
  Read replicas can serve query traffic without affecting ingestion throughput on the primary node.

For deployment patterns and architecture examples, see the [**Disaster Recovery Guide**](./guides/disaster-recovery.mdx).
...
A user can explicitly disable token authorization with `RS_DISABLE_AUTH=true` and use the storage without authentication. In this case, the storage engine doesn't check the token and allows access to all data.
