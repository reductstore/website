...
![An example of an ReductStore instance with two buckets.
](./img/buckets.png)
...
The entries responsible for batching data and storing it in blocks.
...
- **You can read, write, update or delete a batch of records in a single request. This is particularly useful if you need to read or write a large number of small records and reduce the number of requests to the storage engine.
...
You can manage replications by using the [**Replication API**](./http-api/replication-api) or provision them with environment variables. See the [**Replication Guide**](./guides/data-replication) for more details.
