...
A background task that periodically applies an action (such as deleting old records) to a bucket. Lifecycle policies can be scoped to specific entries and filtered with a conditional query.
