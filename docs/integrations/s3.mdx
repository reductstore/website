...
- Put both nodes behind a load balancer that checks `GET /api/v1/ready` (see **[Server API](../http-api/server-api#check-if-the-storage-engine-is-active)**). Route traffic only to nodes returning `200 OK`; a node that loses the lock will return `503` and should be taken out of rotation.
