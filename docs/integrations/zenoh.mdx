...
ReductStore can receive and serve time-series data directly over [**Zenoh**](https://zenoh.io), using its native pub/sub and query primitives. This is an alternative ingestion and retrieval path that runs alongside the existing HTTP API.
...
The Zenoh API is configured through environment variables. For the full variable reference see the **[Zenoh API Settings](../configuration/settings#zenoh-api-settings)** section of the configuration guide.

Enable the API by setting `RS_ZENOH_ENABLED=true`, then provide exactly one of the two session config options below.

### Inline session config

`RS_ZENOH_CONFIG` accepts a semicolon-separated list of `key=value` pairs:
...
Unknown keys are logged as warnings and ignored. For advanced session settings such as TLS, multicast discovery, or scouting, use `RS_ZENOH_CONFIG_PATH` instead and point it to a standard [**Zenoh JSON5 configuration file**](https://github.com/eclipse-zenoh/zenoh/blob/main/DEFAULT_CONFIG.json5).

### Key expression routing

`RS_ZENOH_SUB_KEYEXPRS` sets the key expression the subscriber listens on (write path) and `RS_ZENOH_QUERY_KEYEXPRS` sets the one the queryable answers (read path). Both follow the [**Key Expression Language**](#key-expressions) described above. Either path is disabled when its variable is unset.
...
Zenoh's TLS and user-password authentication normally require certificate and dictionary files to be present on disk. ReductStore provides environment variables that let you supply those file contents directly, so you do not need to mount extra files into the container.

Each variable is written to a temporary file at startup and its path is automatically injected into the Zenoh session config:

| Name                        | Injected Zenoh config key                | Description                                                              |
| --------------------------- | ---------------------------------------- | ------------------------------------------------------------------------ |
| `RS_ZENOH_TLS_ROOT_CA`      | `transport/link/tls/root_ca_certificate` | PEM content of the CA certificate that verifies the Zenoh router or peer |
| `RS_ZENOH_TLS_CONNECT_CERT` | `transport/link/tls/connect_certificate` | PEM content of the client certificate for mutual TLS                     |
| `RS_ZENOH_TLS_CONNECT_KEY`  | `transport/link/tls/connect_private_key` | PEM content of the client private key for mutual TLS                     |
| `RS_ZENOH_AUTH_DICTIONARY`  | `transport/auth/usrpwd/dictionary_file`  | User-password dictionary, one `user:password` per line                   |

These variables work alongside `RS_ZENOH_CONFIG`. The inline config string still enables TLS or user-password auth in the Zenoh session; the credential variables then provide the actual key material without requiring mounted files.

**Example — TLS with inline certificates (Docker):**

```bash
docker run -d \
  -p 8383:8383 \
  -e RS_ZENOH_ENABLED=true \
  -e RS_ZENOH_CONFIG="mode=client;peer=tls/192.168.1.10:7447" \
  -e RS_ZENOH_BUCKET=sensors \
  -e RS_ZENOH_SUB_KEYEXPRS="**" \
  -e RS_ZENOH_TLS_ROOT_CA="$(cat ca.pem)" \
  -e RS_ZENOH_TLS_CONNECT_CERT="$(cat client.pem)" \
  -e RS_ZENOH_TLS_CONNECT_KEY="$(cat client.key)" \
  reduct/store:latest
```

**Example — user-password authentication (Docker):**
...
docker run -d \
  ...
  -e RS_ZENOH_BUCKET=sensors \
  -e RS_ZENOH_SUB_KEYEXPRS="**" \
  -e RS_ZENOH_AUTH_DICTIONARY="alice:s3cr3t
bob:hunter2" \
  reduct/store:latest
```
