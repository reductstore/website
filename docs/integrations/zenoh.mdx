...
| **Queryable**      | Read path. Queries whose selector matches the configured key expression retrieve records.                     |
| **Key**            | The concrete address of a sample (e.g. `robot/arm/joint1`). Becomes the **entry name** in the bucket.       |
