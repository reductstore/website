...
| `each_s`        | float  | Return at most one record every N seconds (must be greater than 0).            |
| `each_n`        | uint64 | Return every Nth record (must be greater than 0).                              |
...
| `metadata`      | bool   | Return labels and timestamps only, without payload.                            |
