...
Key expressions are the addressing language of Zenoh. ReductStore uses the same [Key Expression Language](https://github.com/eclipse-zenoh/roadmap/blob/main/rfcs/ALL/Key%20Expressions.md) for both `RS_ZENOH_SUB_KEYEXPRS` and `RS_ZENOH_QUERY_KEYEXPRS`, and the matched key expression becomes the entry name stored in ReductStore.

A Zenoh key is a `/`-separated sequence of non-empty UTF-8 segments, similar to a filesystem path. Individual key segments may not contain `*`, `$`, `?`, or `#`.

Key expressions extend plain keys with three wildcards:
...
When querying, Zenoh uses a **selector** — a key expression optionally followed by `?` and a list of parameters:
...
The query takes the form of a [selector](#selectors): the key expression maps to the entry name within `RS_ZENOH_BUCKET`, and the parameters after `?` control which records are returned.
