...
- **Verify SSL**: enabled by default. Keep it on for production and disable it only for testing or when you explicitly want to skip TLS certificate verification
- **CA Certificate Path**: optional path to a PEM-encoded CA certificate on the Grafana host. Use this when your ReductStore server uses a private or self-signed certificate authority
...
:::tip
For self-signed HTTPS setups, prefer setting **CA Certificate Path** and keeping **Verify SSL** enabled. Disabling SSL verification is less secure and should only be used for temporary testing.
:::
...
The plugin includes a query builder where you select the bucket, entry, and mode (labels, content, or both). You can also combine the results into one frame. There is a JSON editor available for writing [**Conditional Queries**](/docs/next/conditional-query) that allow you to filter records based on labels and extract specific fields from record contents.
...
The **Combine frames** option is available in plugin version 1.3.0 and later. It
controls how the plugin structures query results and is disabled by default to
preserve the behavior of existing dashboards.

When **Combine frames** is disabled, the plugin returns each label or flattened
JSON value in a separate time-series frame. Enable it when a panel or Grafana
transformation needs the complete record context in one table. The combined
result contains one row per record and always includes:

- `time`: the record timestamp;
- `entry`: the entry that contains the record; and
- value columns selected by the query scope.

The query scope determines the value columns:

- **Label Only** adds record labels.
- **Content Only** adds fields from JSON record content.
- **Label & Content** adds both labels and JSON fields.

The plugin flattens nested JSON objects and arrays into column names such as
`$.temperature` and `$[0].value`. It builds one stable schema for the complete
result, so a field that is absent from a record has a null value in that row.
Labels are converted to numeric or Boolean columns when possible and otherwise
remain strings.

If a label name conflicts with a content column or with `time` or `entry`, the
plugin prefixes the label column with `label.`. It adds a numeric suffix when
needed to keep every column name unique.

:::note
Only JSON object or array content is expanded into combined value columns.
Binary, text, scalar JSON, or empty content still contributes its `time`,
`entry`, and any selected labels, but it does not add content columns. Use an
extension such as
[**ReductSelect**](/docs/extensions/official/select-ext) to return JSON before
combining content fields.
:::
...
## Visualize ROS GPS data

For a complete example that starts a preconfigured Grafana container, queries the
public ROS dataset, and plots interval-averaged GPS coordinates, see the
**[Using Grafana with ROS guide](../../guides/apps/ros/using-grafana.mdx)**.
...
datasources:
  - name: reductstore
    ...
    jsonData:
      ...
      verifySSL: true
      caCertPath: /etc/ssl/certs/reductstore-ca.pem
    secureJsonData:
...
| `verifySSL`   | Enable TLS certificate verification. Defaults to `true` when omitted | No       |
| `caCertPath`  | Path to a PEM-encoded CA certificate file on the Grafana host        | No       |
