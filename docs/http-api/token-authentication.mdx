...
:::info
The API is available when authentication is enabled with `RS_API_TOKEN` or `RS_INIT_API_TOKEN`. It is unavailable when `RS_DISABLE_AUTH=true`.
:::
...
The method creates a new access token from a JSON request body. The request can
include permissions and additional token settings such as expiration, inactivity
TTL, and an IP allowlist. To use this method, you need an access token with
full access.
  ...
  parameters={[
    ...
    },
    {
      ...
      details: {
        ...
        isRequired: false,
        description:
          "Permissions for the token. Includes full_access, read, and write.",
      },
    },
    {
      type: "body",
