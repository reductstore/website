...
### `ttl` and `is_expired` fields

- `ttl` can be provided on `POST /api/v1/tokens/:token_name` to enable inactivity expiry.
- When set, the token is considered expired if `now - last_access > ttl`.
- Read endpoints include `is_expired` so clients can identify unusable tokens:
  - `GET /api/v1/tokens`
  - `GET /api/v1/tokens/:token_name`
  - `GET /api/v1/me`

`is_expired` is informational; authentication checks remain authoritative.
