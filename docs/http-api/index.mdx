...
:::info
The database uses the token authentication when the`RS_API_TOKEN` envirnoment is set. You should use it as a full **[access token](./glossary#access-token)** to create other tokens with different permission by using the Token API
:::
