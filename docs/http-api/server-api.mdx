...
If the **[RS_LOCK_FILE_ENABLED](../configuration/settings.mdx#lock-file-settings)** environment variable is set to `true`, the storage engine starts using a lock file mechanism to prevent multiple instances from accessing the same data directory simultaneously.
