...
The only difference is the entry path: set `:entry_name` to `<entry_name>/$meta` (for example `camera-1/$meta`).
...
- `x-reduct-label-remove: true` is not allowed. Use `PATCH` to remove an attachment.
- Writing with the same `key` replaces the previous attachment with that `key`.


The attachments can be also created with batch endpoints, but the same rules apply: the entry path must end with `/$meta`, and the `key` label is required for each attachment record.we
  ...
  parameters={[
    ...
    },
    {
      ...
      details: {
        name: ":entry_name",
        description:
          "Entry path, including `/$meta` suffix (e.g. `camera-1/$meta`)",
        isRequired: true,
      },
    },
    {
...
Reading attachments works exactly like reading regular records: use the attachment entry path and the timestamp.

`$meta` is a system entry, so it is hidden from bucket `entry_count` and entry lists. It's also impossible to query them with wildcards. You can read them only by their exact entry path (`<entry_name>/$meta`) and timestamp.
