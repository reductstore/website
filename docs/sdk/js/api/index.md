---
title: "reduct-js"
description: "API reference for reduct-js in the ReductStore Client SDK for JavaScript."
---
<head>
  <link rel="canonical" href="https://www.reduct.store/docs/sdk/js/api/index" />
</head>


&lt;head&gt;
  &lt;link rel="canonical" href="https://www.reduct.store/docs/sdk/js/api/index" /&gt;
&lt;/head&gt;

# reduct-js

## Namespaces

- [LifecycleMode](reduct-js/namespaces/LifecycleMode/index.md)
- [LifecycleType](reduct-js/namespaces/LifecycleType/index.md)
- [ReplicationCompression](reduct-js/namespaces/ReplicationCompression/index.md)
- [ReplicationMode](reduct-js/namespaces/ReplicationMode/index.md)

## Enumerations

- [QuotaType](enumerations/QuotaType.md)
- [RecordBatchType](enumerations/RecordBatchType.md)
- [Status](enumerations/Status.md)

## Classes

- [APIError](classes/APIError.md)
- [Batch](classes/Batch.md)
- [Bucket](classes/Bucket.md)
- [BucketInfo](classes/BucketInfo.md)
- [BucketSettings](classes/BucketSettings.md)
- [Client](classes/Client.md)
- [EntryInfo](classes/EntryInfo.md)
- [FullLifecycleInfo](classes/FullLifecycleInfo.md)
- [FullReplicationInfo](classes/FullReplicationInfo.md)
- [LicenseInfo](classes/LicenseInfo.md)
- [LifecycleInfo](classes/LifecycleInfo.md)
- [LifecycleSettings](classes/LifecycleSettings.md)
- [QueryOptions](classes/QueryOptions.md)
- [RecordBatch](classes/RecordBatch.md)
- [ReplicationInfo](classes/ReplicationInfo.md)
- [ReplicationSettings](classes/ReplicationSettings.md)
- [ServerInfo](classes/ServerInfo.md)
- [Token](classes/Token.md)
- [TokenCreateRequest](classes/TokenCreateRequest.md)
- [TokenPermissions](classes/TokenPermissions.md)

## Interfaces

- [WriteOptions](interfaces/WriteOptions.md)

## Type Aliases

- [LifecycleMode](type-aliases/LifecycleMode.md)
- [LifecycleType](type-aliases/LifecycleType.md)
- [ReplicationCompression](type-aliases/ReplicationCompression.md)
- [ReplicationMode](type-aliases/ReplicationMode.md)
