...
<a id="reduct.bucket.Bucket.remove_record_batch"></a>

#### remove\_record\_batch

```python
async def remove_record_batch(
        batch: RecordBatch) -> dict[str, dict[int, ReductError]]
```
...
```python
async def remove_query(entries: str | list[str],
...
<a id="reduct.bucket.Bucket.write_record_batch"></a>

#### write\_record\_batch
...
<a id="reduct.bucket.Bucket.update_record_batch"></a>

#### update\_record\_batch
...
```python
async def query(entries: str | list[str],
                start: TimestampLike | None = None,
                stop: TimestampLike | None = None,
                ttl: int | None = None,
                when: dict | None = None,
...
```python
async def subscribe(entries: str | list[str],
                    start: TimestampLike | None = None,
                    poll_interval=1.0,
                    when: dict | None = None,
                    **kwargs) -> AsyncIterator[Record]
...
```python
async def create_query_link(entries: str | list[str],
                            start: TimestampLike | None = None,
                            stop: TimestampLike | None = None,
                            when: dict | None = None,
                            **kwargs) -> str
...
<a id="reduct.bucket.Bucket.write_attachments"></a>

#### write\_attachments

```python
async def write_attachments(entry_name: str, attachments: dict[str, dict])
```
...
<a id="reduct.bucket.Bucket.read_attachments"></a>

#### read\_attachments

```python
async def read_attachments(entry_name: str) -> dict[str, Any]
```
...
<a id="reduct.bucket.Bucket.remove_attachments"></a>

#### remove\_attachments

```python
async def remove_attachments(entry_name: str,
