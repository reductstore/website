...
func main() {
	...
	err = writer.Write("<Blob data>", -1)
	if err != nil {
		panic(err)
	}
	writer = bucket.BeginWrite(ctx, "entry-1", &reduct.WriteOptions{Timestamp: ts + 1, Labels: map[string]any{"score": 20}})
	err = writer.Write("<Blob data 2>", -1)
	if err != nil {
		panic(err)
	}

	// 4. Query the data by time range and condition
	queryOptions := &reduct.QueryOptions{
		Start: &ts,
		When: map[string]any{
			"&score": map[string]any{"$gt": 10},
		},
	}
	...
	for rec := range records.Records() {
		...
		timestamp := rec.Time()
		labels := rec.Labels()
		println("Record at time:", timestamp, "with labels:", labels, "and data:", string(data))
	}
}
