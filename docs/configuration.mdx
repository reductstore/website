...
In addition to general settings, you can configure I/O settings to optimize communication over HTTP with the storage engine. The following table describes the available environment variable:
...
However, if a client is using the HTTP/1.1 protocol, the `RS_IO_BATCH_MAX_METADATA_SZE` and `RS_IO_BATCH_MAX_RECORDS` settings are used to limit the size of the headers in the HTTP request from the client.
This means that the client cannot send more records in a single batch than specified by the `RS_IO_BATCH_MAX_RECORDS`and the size of the headers in the request can't exceed the value of the `RS_IO_BATCH_MAX_METADATA_SIZE` setting.
