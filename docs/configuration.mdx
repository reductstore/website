...
| `RS_REMOTE_REGION`       |             | Region of the S3-compatible object storage. It's ignored for on-premise storage like MinIO. |
...
By default, ReductStore uses the local file system to store data. If you want to use a remote backend, you need to set the `RS_REMOTE_BACKEND_TYPE` variable to `S3` and configure the other variables accordingly.
